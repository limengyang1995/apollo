...
bool ControlComponent::Init() {
    ...
    cyber::ReaderConfig cloudcontrol_reader_config;
    cloudcontrol_reader_config.channel_name = FLAGS_cloud_control_command_topic;
    cloudcontrol_reader_config.pending_queue_size = FLAGS_cloudcontrol_pending_queue_size;

    cloud_control_command_reader_ = node_->CreateReader<ControlCommand>(cloudcontrol_reader_config, nullptr);
    ACHECK(cloud_control_command_reader_ != nullptr);
...
void ControlComponent::OnCloudControlCommand(const ControlCommand &cloud_control_command) {
...
void ControlComponent::ResetAndProduceZeroControlCommand(ControlCommand *control_command) {
    ...
    control_command->set_gear_location(Chassis::GEAR_DRIVE);
    control_command->set_low_beam(0);
...
void ControlComponent::GetVehiclePitchAngle(ControlCommand *control_command) {
    ...
    control_command->mutable_debug()->mutable_simple_lon_debug()->set_vehicle_pitch(
            vehicle_pitch + FLAGS_pitch_offset_deg);
}
