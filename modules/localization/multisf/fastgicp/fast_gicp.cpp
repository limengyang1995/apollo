...
template <typename PointSource, typename PointTarget, typename SearchMethodSource, typename SearchMethodTarget>
bool FastGICP<PointSource, PointTarget, SearchMethodSource, SearchMethodTarget>::Get_calculate_covariances(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& cloud,
  pcl::search::Search<pcl::PointXYZ>& kdtree,
  std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& covariances){
  calculate_covariances(cloud,kdtree,covariances);
  return true;
}
// 显式实例化
template class FastGICP<pcl::PointXYZ, pcl::PointXYZ>; // 根据需要实例化类型
}  // namespace fast_gicp
