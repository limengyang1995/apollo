...
template <typename PointSource, typename PointTarget>
void FastVGICP<PointSource, PointTarget>::InPutCovariances(const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>covariances){
  target_covs_ = covariances;
}
...
// 显式实例化
template class FastVGICP<pcl::PointXYZ, pcl::PointXYZ>; // 根据需要实例化类型
}  // namespace fast_gicp
