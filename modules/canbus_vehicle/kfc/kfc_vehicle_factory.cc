...
#include "modules/canbus_vehicle/kfc/kfc_vehicle_factory.h"
...
bool KfcVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
  ...
  chassis_detail_writer_ =
      node_->CreateWriter<::apollo::canbus::Kfc>(FLAGS_chassis_detail_topic);

  return true;
}

bool KfcVehicleFactory::Start() {
...
Chassis KfcVehicleFactory::publish_chassis() {
  Chassis chassis = vehicle_controller_->chassis();
  ADEBUG << chassis.ShortDebugString();
  return chassis;
}

void KfcVehicleFactory::PublishChassisDetail() {
  Kfc chassis_detail;
...
void KfcVehicleFactory::UpdateHeartbeat() {
    can_sender_.Update_Heartbeat();
}

std::unique_ptr<VehicleController<::apollo::canbus::Kfc>>
KfcVehicleFactory::CreateVehicleController() {
  return std::unique_ptr<VehicleController<::apollo::canbus::Kfc>>(
      new kfc::KfcController());
}

std::unique_ptr<MessageManager<::apollo::canbus::Kfc>>
KfcVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Kfc>>(
      new kfc::KfcMessageManager());
}
