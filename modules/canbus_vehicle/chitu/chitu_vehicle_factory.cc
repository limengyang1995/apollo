...
#include "modules/canbus_vehicle/chitu/chitu_vehicle_factory.h"
...
bool ChituVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
  ...
  chassis_detail_writer_ =
      node_->CreateWriter<::apollo::canbus::Chitu>(FLAGS_chassis_detail_topic);

  chassis_detail_sender_writer_ = node_->CreateWriter<::apollo::canbus::Chitu>(
      FLAGS_chassis_detail_sender_topic);

  return true;
}

bool ChituVehicleFactory::Start() {
...
void ChituVehicleFactory::UpdateCommand(
...
Chassis ChituVehicleFactory::publish_chassis() {
  Chassis chassis = vehicle_controller_->chassis();
  ADEBUG << chassis.ShortDebugString();
  return chassis;
}

void ChituVehicleFactory::PublishChassisDetail() {
  Chitu chassis_detail = vehicle_controller_->GetNewRecvChassisDetail();
  ADEBUG << "latest chassis_detail is " << chassis_detail.ShortDebugString();
  chassis_detail_writer_->Write(chassis_detail);
}

void ChituVehicleFactory::PublishChassisDetailSender() {
  Chitu sender_chassis_detail = vehicle_controller_->GetNewSenderChassisDetail();
...
void ChituVehicleFactory::UpdateHeartbeat() {
    can_sender_.Update_Heartbeat();
}

bool ChituVehicleFactory::CheckChassisCommunicationFault() {
...
void ChituVehicleFactory::AddSendProtocol() {
  vehicle_controller_->AddSendMessage();
}

void ChituVehicleFactory::ClearSendProtocol() {
  can_sender_.ClearMessage();
}

bool ChituVehicleFactory::IsSendProtocolClear() {
...
Chassis::DrivingMode ChituVehicleFactory::Driving_Mode() {
  return vehicle_controller_->driving_mode();
}

std::unique_ptr<VehicleController<::apollo::canbus::Chitu>>
ChituVehicleFactory::CreateVehicleController() {
  return std::unique_ptr<VehicleController<::apollo::canbus::Chitu>>(
      new chitu::ChituController());
}

std::unique_ptr<MessageManager<::apollo::canbus::Chitu>>
ChituVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Chitu>>(
      new chitu::ChituMessageManager());
}
