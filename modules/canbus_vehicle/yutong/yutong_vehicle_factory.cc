...
#include "modules/canbus_vehicle/yutong/yutong_vehicle_factory.h"
...
bool YutongVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
  ...
  chassis_detail_writer_ =
      node_->CreateWriter<::apollo::canbus::Yutong>(FLAGS_chassis_detail_topic);

  return true;
}

bool YutongVehicleFactory::Start() {
...
void YutongVehicleFactory::Stop() {
  can_sender_.Stop();
  can_receiver_.Stop();
  can_client_->Stop();
...
void YutongVehicleFactory::UpdateCommand(
...
Chassis YutongVehicleFactory::publish_chassis() {
  Chassis chassis = vehicle_controller_->chassis();
  ADEBUG << chassis.ShortDebugString();
  return chassis;
}

void YutongVehicleFactory::PublishChassisDetail() {
...
void YutongVehicleFactory::UpdateHeartbeat() {
    can_sender_.Update_Heartbeat();
}

std::unique_ptr<VehicleController<::apollo::canbus::Yutong>>
YutongVehicleFactory::CreateVehicleController() {
  return std::unique_ptr<VehicleController<::apollo::canbus::Yutong>>(
      new yutong::YutongController());
}

std::unique_ptr<MessageManager<::apollo::canbus::Yutong>>
YutongVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Yutong>>(
      new yutong::YutongMessageManager());
}
