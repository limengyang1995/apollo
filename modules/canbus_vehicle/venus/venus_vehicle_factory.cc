...
#include "modules/canbus_vehicle/venus/venus_vehicle_factory.h"
...
bool VenusVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
  ...
  chassis_detail_writer_ =
      node_->CreateWriter<::apollo::canbus::Venus>(FLAGS_chassis_detail_topic);

  return true;
}

bool VenusVehicleFactory::Start() {
...
void VenusVehicleFactory::UpdateCommand(
...
Chassis VenusVehicleFactory::publish_chassis() {
  Chassis chassis = vehicle_controller_->chassis();
  ADEBUG << chassis.ShortDebugString();
  return chassis;
}

void VenusVehicleFactory::PublishChassisDetail() {
...
void VenusVehicleFactory::UpdateHeartbeat() {
    can_sender_.Update_Heartbeat();
}

std::unique_ptr<VehicleController<::apollo::canbus::Venus>>
VenusVehicleFactory::CreateVehicleController() {
  return std::unique_ptr<VehicleController<::apollo::canbus::Venus>>(
      new venus::VenusController());
}

std::unique_ptr<MessageManager<::apollo::canbus::Venus>>
VenusVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Venus>>(
      new venus::VenusMessageManager());
}
