...
ErrorCode VenusController::Init(
    ...
    // sender part
    acu1_529_ = dynamic_cast<Acu1529*>(message_manager_->GetMutableProtocolDataById(Acu1529::ID));
    if (acu1_529_ == nullptr) {
        AERROR << "Acu1529 does not exist in the VenusMessageManager!";
        return ErrorCode::CANBUS_ERROR;
    }
    acu2_532_ = dynamic_cast<Acu2532*>(message_manager_->GetMutableProtocolDataById(Acu2532::ID));
    if (acu2_532_ == nullptr) {
        AERROR << "Acu2532 does not exist in the VenusMessageManager!";
        return ErrorCode::CANBUS_ERROR;
    }
    acu3_534_ = dynamic_cast<Acu3534*>(message_manager_->GetMutableProtocolDataById(Acu3534::ID));
...
Chassis VenusController::chassis() {
    ...
    if (chassis_detail.has_vcu3_2_56d()) {
        ...
        if (vcu3_2_56d.has_vcu3_wheelspeedrr()) {
            ...
            chassis_.mutable_wheel_speed()->set_wheel_spd_rr(vcu3_2_56d.vcu3_wheelspeedrr());
        }

        if (vcu3_2_56d.has_vcu3_wheelspeedrl()) {
            chassis_.mutable_wheel_speed()->set_is_wheel_spd_rl_valid(true);
            chassis_.mutable_wheel_speed()->set_wheel_spd_rl(vcu3_2_56d.vcu3_wheelspeedrl());
        }

        if (vcu3_2_56d.has_vcu3_wheelspeedfr()) {
            chassis_.mutable_wheel_speed()->set_is_wheel_spd_fr_valid(true);
            chassis_.mutable_wheel_speed()->set_wheel_spd_fr(vcu3_2_56d.vcu3_wheelspeedfr());
        }

        if (vcu3_2_56d.has_vcu3_wheelspeedfl()) {
            chassis_.mutable_wheel_speed()->set_is_wheel_spd_fl_valid(true);
            chassis_.mutable_wheel_speed()->set_wheel_spd_fl(vcu3_2_56d.vcu3_wheelspeedfl());
        }
    }
    // 6 vehiclespeed  m/s
    if (chassis_detail.has_vcu1_56f() && chassis_detail.vcu1_56f().has_vcu1_vehiclespeed()) {
        chassis_.set_speed_mps(static_cast<float>(chassis_detail.vcu1_56f().vcu1_vehiclespeed() / 3.6));
    } else {
    ...
    if (chassis_detail.has_vcu6_56c()) {
        if (chassis_detail.vcu6_56c().has_vcu6_batterysoc()) {
            chassis_.set_battery_soc_percentage(static_cast<int>(chassis_detail.vcu6_56c().vcu6_batterysoc()));
        } else {
            chassis_.set_battery_soc_percentage(0);
        }
        if (chassis_detail.vcu6_56c().has_vcu6_throttleposition()) {
            chassis_.set_throttle_percentage(static_cast<float>(chassis_detail.vcu6_56c().vcu6_throttleposition()));
        } else {
            chassis_.set_throttle_percentage(0);
        }
        if (chassis_detail.vcu6_56c().has_vcu6_brakeposition()) {
            chassis_.set_brake_percentage(static_cast<float>(chassis_detail.vcu6_56c().vcu6_brakeposition()));
        } else {
...
void VenusController::Gear(Chassis::GearPosition gear_position) {
    ...
    if (chassis_detail.has_vcu5_56a() && chassis_detail.vcu5_56a().has_vcu5_gearautocontrolst()
...
void VenusController::Steer(double angle) {
     ...
     */
    acu1_529_->set_acu1_steeringautocontrol(Acu1_529::ACU1_STEERINGAUTOCONTROL_REQUEST);
    acu1_529_->set_acu1_targetsteeringangle_f(
            double((-1) * vehicle_params_.max_steer_angle() / M_PI * 180.0 * angle / 100.0));
    ...
    if (chassis_detail.has_vcu5_56a()) {
        ...
        if (chassis_detail.vcu5_56a().has_vcu5_frontsteeringfailurest()
            ...
            if (chassis_detail.vcu5_56a().vcu5_frontsteeringfailurest() != Vcu5_56a::VCU5_FRONTSTEERINGFAILUREST_NOTFAIL
                && chassis_detail.vcu5_56a().vcu5_rearsteeringfailurest()
                        != Vcu5_56a::VCU5_REARSTEERINGFAILUREST_NOTFAIL) {
...
void VenusController::SecurityDogThreadFunc() {
    ...
    while (can_sender_->IsRunning()) {
        ...
        if (elapsed < default_period) {
            ...
            AERROR << "Too much time consumption in VenusController looping process:" << elapsed.count();
        }
    }
}
...
bool VenusController::CheckResponse(const int32_t flags, bool need_wait) {
    ...
    do {
        ...
        if (flags & CHECK_RESPONSE_SPEED_UNIT_FLAG) {
            is_vcu_online = chassis_detail.has_vcu5_56a() && chassis_detail.vcu5_56a().has_vcu5_drivingtakeoverst()
                    && !chassis_detail.vcu5_56a().vcu5_drivingtakeoverst();
            is_esp_online = chassis_detail.has_vcu5_56a() && chassis_detail.vcu5_56a().has_vcu5_brakingtakeoverst()
    ...
    AERROR << "check_response fail: is_eps_online:" << is_eps_online << ", is_vcu_online:" << is_vcu_online
...
void VenusController::set_chassis_error_code(const Chassis::ErrorCode& error_code) {
    std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
