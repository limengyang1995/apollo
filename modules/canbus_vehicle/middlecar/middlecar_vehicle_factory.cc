...
#include "modules/canbus_vehicle/middlecar/middlecar_vehicle_factory.h"
...
bool MiddlecarVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
  ...
  chassis_detail_writer_ =
      node_->CreateWriter<::apollo::canbus::Middlecar>(FLAGS_chassis_detail_topic);

  chassis_detail_sender_writer_ = node_->CreateWriter<::apollo::canbus::Middlecar>(
      FLAGS_chassis_detail_sender_topic);

  return true;
}

bool MiddlecarVehicleFactory::Start() {
...
void MiddlecarVehicleFactory::Stop() {
  can_sender_.Stop();
  can_receiver_.Stop();
  can_client_->Stop();
...
void MiddlecarVehicleFactory::UpdateCommand(
...
Chassis MiddlecarVehicleFactory::publish_chassis() {
  Chassis chassis = vehicle_controller_->chassis();
  ADEBUG << chassis.ShortDebugString();
  return chassis;
}

void MiddlecarVehicleFactory::PublishChassisDetail() {
  Middlecar chassis_detail = vehicle_controller_->GetNewRecvChassisDetail();
  ADEBUG << "latest chassis_detail is " << chassis_detail.ShortDebugString();
  chassis_detail_writer_->Write(chassis_detail);
}

void MiddlecarVehicleFactory::PublishChassisDetailSender() {
  Middlecar sender_chassis_detail = vehicle_controller_->GetNewSenderChassisDetail();
...
void MiddlecarVehicleFactory::UpdateHeartbeat() {
    can_sender_.Update_Heartbeat();
}

bool MiddlecarVehicleFactory::CheckChassisCommunicationFault() {
...
void MiddlecarVehicleFactory::AddSendProtocol() {
  vehicle_controller_->AddSendMessage();
}

void MiddlecarVehicleFactory::ClearSendProtocol() {
  can_sender_.ClearMessage();
}

bool MiddlecarVehicleFactory::IsSendProtocolClear() {
...
Chassis::DrivingMode MiddlecarVehicleFactory::Driving_Mode() {
  return vehicle_controller_->driving_mode();
}

std::unique_ptr<VehicleController<::apollo::canbus::Middlecar>>
MiddlecarVehicleFactory::CreateVehicleController() {
  return std::unique_ptr<VehicleController<::apollo::canbus::Middlecar>>(
      new middlecar::MiddlecarController());
}

std::unique_ptr<MessageManager<::apollo::canbus::Middlecar>>
MiddlecarVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Middlecar>>(
      new middlecar::MiddlecarMessageManager());
}
