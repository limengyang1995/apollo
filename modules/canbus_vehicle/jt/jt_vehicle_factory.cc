...
#include "modules/canbus_vehicle/jt/jt_vehicle_factory.h"
...
bool JtVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
...
void JtVehicleFactory::UpdateCommand(
...
Chassis JtVehicleFactory::publish_chassis() {
...
JtVehicleFactory::CreateVehicleController() {
  return std::unique_ptr<VehicleController<::apollo::canbus::Jt>>(
      new jt::JtController());
}

std::unique_ptr<MessageManager<::apollo::canbus::Jt>>
JtVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Jt>>(
      new jt::JtMessageManager());
}
