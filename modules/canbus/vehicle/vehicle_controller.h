...
private:
     ...
     */
    virtual common::ErrorCode HandleCustomOperation(const external_command::ChassisCommand &command) = 0;

    /*
     * @brief Handle Apollo chassis actions
     */
    virtual void HandleVehicleSignal(const common::VehicleSignal &signal) {
        if (signal.has_high_beam() || signal.has_low_beam())
            SetBeam(signal);
        if (signal.has_horn())
            SetHorn(signal);
        if (signal.has_turn_signal())
            SetTurningSignal(signal);
    }
    ...
    virtual common::VehicleSignal ProcessCommandChange(
            const common::VehicleSignal &signal,
...
bool VehicleController<SensorType>::CheckChassisCommunicationError() {
    ...
    if (message_manager_->GetSensorCheckSenderData(&chassis_detail_sender) != ErrorCode::OK) {
        AERROR_EVERY(100) << "Get " << typeid(SensorType).name() << " chassis receive detail failed.";
    }
    ...
    if (message_manager_->GetSensorCheckRecvData(&chassis_detail_receiver) != ErrorCode::OK) {
        AERROR_EVERY(100) << "Get " << typeid(SensorType).name() << " chassis receive detail failed.";
    }
    ADEBUG << "chassis_detail_receiver is " << chassis_detail_receiver.ShortDebugString();
    ...
    ADEBUG << "lost_chassis_reveive_detail_count_ is " << lost_chassis_reveive_detail_count_;
...
SensorType VehicleController<SensorType>::GetNewRecvChassisDetail() {
    ...
    if (message_manager_->GetSensorRecvData(&receiver_chassis_detail) != ErrorCode::OK) {
        AERROR_EVERY(100) << "Get " << typeid(SensorType).name() << " chassis detail receiver failed.";
        return receiver_chassis_detail;
    }
    ADEBUG << "receiver_chassis_detail is " << receiver_chassis_detail.ShortDebugString();
...
SensorType VehicleController<SensorType>::GetNewSenderChassisDetail() {
    ...
    if (message_manager_->GetSensorSenderData(&sender_chassis_detail) != ErrorCode::OK) {
        AERROR_EVERY(100) << "Get " << typeid(SensorType).name() << " chassis detail receiver failed.";
        return sender_chassis_detail;
    }
    message_manager_->GetSensorSenderData(&sender_chassis_detail);
    ADEBUG << "sender_chassis_detail is " << sender_chassis_detail.ShortDebugString();
...
template <typename SensorType>
void VehicleController<SensorType>::set_driving_mode(const Chassis::DrivingMode &driving_mode) {
    std::lock_guard<std::mutex> lock(mode_mutex_);
...
ErrorCode VehicleController<SensorType>::Update(const ControlCommand &control_command) {
    ...
    // AINFO << "receive control: acc : " << control_command.acceleration() << " steer: " <<
    // control_command.steering_target();

    if (control_command.has_pad_msg() && control_command.pad_msg().has_action()) {
        ADEBUG << "Canbus received pad msg: " << control_command.pad_msg().ShortDebugString();
        ...
        if ((FLAGS_chassis_debug_mode || (pad_msg_time_diff < FLAGS_pad_msg_delay_interval))
            ...
            AERROR << "pad msg time out, current time interval is " << pad_msg_time_diff << " s, threshold is "
    ...
    if ((driving_mode() == Chassis::COMPLETE_AUTO_DRIVE || driving_mode() == Chassis::AUTO_SPEED_ONLY
         || driving_mode() == Chassis::AUTO_STEER_ONLY || driving_mode() == Chassis::REMOTE_CLOUD_DRIVE)
...
template <typename SensorType>
ErrorCode VehicleController<SensorType>::Update(const ChassisCommand &chassis_command) {
    if (!is_initialized_) {
        AERROR << "Controller is not initialized.";
        return ErrorCode::CANBUS_ERROR;
    }
    ...
    if ((driving_mode() == Chassis::COMPLETE_AUTO_DRIVE || driving_mode() == Chassis::AUTO_SPEED_ONLY
        ...
        HandleVehicleSignal(ProcessCommandChange(chassis_command.basic_signal(), &last_chassis_command_));
    }
...
common::VehicleSignal VehicleController<SensorType>::ProcessCommandChange(
    ...
    if (vehicle_signal.has_high_beam()) {
        if ((last_command->has_high_beam() && last_command->high_beam() != vehicle_signal.has_high_beam())
    ...
    if (vehicle_signal.has_low_beam()) {
        if ((last_command->has_low_beam() && last_command->low_beam() != vehicle_signal.has_low_beam())
    ...
    if (vehicle_signal.has_horn()) {
        if ((last_command->has_horn() && last_command->horn() != vehicle_signal.horn()) || !last_command->horn()) {
            vehicle_signal_end.set_horn(vehicle_signal.horn());
        }
    }

    if (vehicle_signal.has_turn_signal()) {
        if ((last_command->has_turn_signal() && last_command->turn_signal() != vehicle_signal.turn_signal())
