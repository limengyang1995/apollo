...
DEFINE_bool(use_acceleration, true, "switch to use acceleration instead of throttle pedal and brake pedal");
...
public:
        ...
        while (IsRunning()) {
            ...
            case KEYCODE_UP2:
                ...
                if (brake > 1e-6) {
                    ...
                    if (!FLAGS_use_acceleration) {
                        ...
                        // control_command_.set_speed(dec);
                        control_command_.set_brake(brake);
                    }
                } else {
                    ...
                    if (!FLAGS_use_acceleration) {
                        ...
                        control_command_.set_speed(acc);
                    }
                }
                if (!FLAGS_use_acceleration) {
                    AINFO << "Throttle = " << control_command_.throttle() << ", Brake = " << control_command_.brake();
                } else {
            ...
            case KEYCODE_SETQ2:
                ...
                if (cnt == 0) {
                    ...
                    control_command_.mutable_signal()->set_turn_signal(VehicleSignal::TURN_HAZARD_WARNING);
    ...
    void GetPadMessage(PadMessage *pad_msg, int32_t int_action) {
        ...
        pad_msg->mutable_header()->set_timestamp_sec(Time::Now().ToSecond());
        return;
    }
    ...
    void ResetControlCommand() {
        ...
        control_command_.set_steering_target(10.0);
    ...
    int32_t Start() {
        ...
        chassis_reader_ = node_->CreateReader<Chassis>(
                FLAGS_chassis_topic, [this](const std::shared_ptr<Chassis> &chassis) { OnChassis(*chassis); });
